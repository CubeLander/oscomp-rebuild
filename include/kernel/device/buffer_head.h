...
struct buffer_head {
	...
	struct blockdevice* b_bdev; /* Block device this buffer is from */
	sector_t b_blocknr;         /* Block number */
	size_t b_size;              /* Block size */
	uint64 b_state;             /* State flags */
	// atomic_t            b_refcount;       /* Reference counter */
	//  在一个简化的设计里，不需要做高性能io，这个字段可以简化掉
...
// 缓冲同步函数组
int32 sync_blockdev(struct blockdevice *bdev);
*/
