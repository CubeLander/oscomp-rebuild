 ...
 */
static inline struct kmalloc_header* ptr_to_header(void* ptr) { return (struct kmalloc_header*)((char*)ptr - HEADER_SIZE); }

/**
 * @brief Get user pointer from header
 */
static inline void* header_to_ptr(struct kmalloc_header* header) { return (void*)((char*)header + HEADER_SIZE); }
...
void* kmalloc(size_t size) {
	...
	if (total_size <= 2048) {
		...
		vaddr_t ret = mmap_file(&init_mm, 0, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_POPULATE, NULL, 0);
...
void* kcalloc(size_t n, size_t size) {
	...
	// 检查乘法溢出
	if (size && n > (~(size_t)0) / size) return NULL;

	total_size = n * size;

	// 如果总大小为0，返回一个特殊值或NULL
	if (total_size == 0) return NULL; // 或者返回一个特殊的非NULL值，取决于您的需求
