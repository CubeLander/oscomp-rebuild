...
void ext4_lock(void) { mutex_lock(&ext4_mtx); }

void ext4_unlock(void) { mutex_unlock(&ext4_mtx); }
...
int32 ext4_fill_super(struct superblock* sb, void* data, int32 silent) {
	...
	struct blockdevice* kernel_bdev = sb->s_bdev;
	if (!kernel_bdev) {
		kfree(e_fs);
		return -EINVAL;
	}
...
struct fstype ext4_fs_type = {
    .fs_name = "ext4", .fs_flags = FS_REQUIRES_DEV,
    //.fs_mount = ext4_adapter_mount,
